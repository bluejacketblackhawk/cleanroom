...
//! force one via `CLEANROOM_H264_ENCODER`, [`FfmpegSidecar::h264_encoder`] refuses.
...
/// Video encoders that force ffmpeg into `--enable-gpl`. Cleanroom is MIT and ships an LGPL sidecar,
...
/// Whether `name` is a GPL-linked video encoder Cleanroom must never use (handoff/07 §2).
...
impl FfmpegSidecar {
    ...
    ///
    /// `CLEANROOM_H264_ENCODER` overrides the search, but never past the licence bar: a GPL-linked
    /// encoder ([`is_gpl_video_encoder`]) is refused outright.
    pub fn h264_encoder(&self) -> Result<String, MediaError> {
        if let Some(forced) = std::env::var("CLEANROOM_H264_ENCODER")
            .ok()
            ...
            .filter(|v| !v.is_empty())
        {
            if is_gpl_video_encoder(&forced) {
                return Err(MediaError::InvalidClip(format!(
                    "refusing to encode with `{forced}`: it is GPL-linked, and Cleanroom only ever \
        ...
        Err(MediaError::SidecarFailed(format!(
            "no usable LGPL-safe H.264 encoder in this ffmpeg build (wanted one of {:?}; \
             advertised but failed to run: {:?}). Cleanroom never falls back to GPL x264 — \
             see handoff/07 §2",
...
fn build_script(words: &[ClipWord], spec: &ClipSpec, duration: f64) -> String {
    ...
    s.push_str(
        "; Generated by Cleanroom Clip Studio — do not edit, it is rewritten every render.\n",
    );
